...
REDIRECT_URI: str = f'http://localhost:{REDIRECT_PORT}/callback' # Must be registered with your provider
...
def generate_pkce_pair() -> Tuple[str, str]:
    ...
    code_verifier: str = secrets.token_urlsafe(96) # Generates 128 characters safe for URL
...
def refresh_access_token(oauth: OAuth2Session, tokens: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    print("Access token expired, refreshing...")
    try:
        data: Dict[str, str] = {
            'grant_type': 'refresh_token',
            'refresh_token': str(tokens['refresh_token']), # Ensure refresh_token is string
            'client_id': CLIENT_ID,
...
def get_authorized_session() -> Optional[OAuth2Session]:
    ...
    oauth: OAuth2Session = OAuth2Session(CLIENT_ID, scope=SCOPES, redirect_uri=REDIRECT_URI)
    ...
    if tokens:
        ...
        if isinstance(expires_at, (int, float)) and expires_at < time.time() - 300: # Give a 5 min buffer
    ...
    if not tokens: # No tokens, or refresh failed, initiate full authorization
        ...
        if not auth_code:
            print("Failed to receive authorization code.") # Removed "within timeout" as serve_forever blocks.
            return None
